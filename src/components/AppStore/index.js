...
class AppStore extends Component {
  ...
  filterResults = () => {
    ...
    const filteredResults = appsList.filter(apps => apps.category === tab)
    return filteredResults
  }

  render() {
    const {search, tab} = this.state
    const filteredResults = this.filterResults().filter(eachApp =>
      ...
      <div className="main-container">
        <div className="container">
          ...
          <ul className="app-items">
            {filteredResults.map(appItem => (
              <AppItem app={appItem} key={appItem.appId} />
            ))}
          </ul>
        </div>
      </div>
